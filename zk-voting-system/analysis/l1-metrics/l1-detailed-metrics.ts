...
class AdvancedPerformanceAnalyzer {
  ...
  private async measureNetworkBaseline(): Promise<any> {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      gasPrices.push(feeData.gasPrice || 0n);
    }
  ...
  private async analyzeDetailedMetrics(
    ...
    const networkHealth = {
      ...
      gasPriceStability: Number(networkBaseline.averageGasPrice) / Number(currentFeeData.gasPrice || 1n),
      networkUtilization: Number(gasPerVote.average) > 50000 ? 0.8 : 0.3 // Rough estimate
    };
